...
def combat(data, batch, model=None, numerical_covariates=None):
    ...
    for i, batch_idxs in enumerate(batch_info):
        ...
        delta_hat.append(s_data[batch_idxs].var(axis=1).fillna(0))
    ...
    delta_star = np.array(delta_star).astype(np.float64)
    ...
    for j, batch_idxs in enumerate(batch_info):
        ...
        bayesdata[batch_idxs] = np.nan_to_num(bayesdata[batch_idxs])
...
def aprior(gamma_hat):
    m = gamma_hat.mean()
    s2 = gamma_hat.var()
    return np.nan_to_num((2 * s2 + m ** 2) / s2)


def bprior(gamma_hat):
    m = gamma_hat.mean()
    s2 = gamma_hat.var()
    return np.nan_to_num((m * s2 + m ** 3) / s2)
...
if __name__ == "__main__":
    ...
    for dataset in ["COMB"]:
        ...
        pheno = pd.read_table(os.path.join(constants.DATA_DIR, phenotype_file_name), index_col=0)
        dat = pd.read_table(os.path.join(constants.DATA_DIR, gene_expression_file_name), index_col=0, dtype="str")
        print "done load"
