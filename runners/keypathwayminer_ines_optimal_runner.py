...
def extract_module_genes(bg_genes, STRATEGY, algorithm):
    i = 1
    modules = []
    while os.path.exists(os.path.join(ALGO_DIR, "results", "Pathway-{}-NODES-.txt".format("%01d" % (i,)))) and i<2:
        results = file(
            os.path.join(ALGO_DIR, "results", "Pathway-{}-NODES-.txt".format("%01d" % (i,)))).readlines()
