...
class CancerTypesDataset(Dataset):
    ...
    def __init__(self, csv_files, labels=None):
        ...
        for csv_file, csv_label in zip(csv_files, labels):
            ...
            self.labels = np.append(self.labels, [csv_label for x in range(self.samples.shape[0])])
    ...
    def __getitem__(self, idx):
        labels=np.array(['LUSC', 'LUAD'])
        return torch.tensor(self.samples.iloc[idx,:].values, dtype=torch.float), torch.tensor((labels==list(self.labels)[idx]).astype(np.int), dtype=torch.long)
